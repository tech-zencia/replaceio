...
import { MapPin, BedDouble, Bath, Maximize2, Phone, Heart, ArrowLeft, Loader2, CheckSquare } from "lucide-react";
...
export default function PropertyDetail() {
  ...
  if (!property) return (
    <div className="text-center py-20 text-gray-500">Property not found.</div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-5">
      {/* Back */}
      <Link to="/search" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 mb-4">
          ...
          <div className="flex gap-2 p-3 overflow-x-auto">
            {images.map((m, i) => (
              <button key={m.id} onClick={() => setImageIdx(i)}
                className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-colors ${i === imageIdx ? "border-brand-500" : "border-transparent"}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
          <div>
            <div className="flex items-start justify-between gap-3">
              ...
              <button onClick={handleSave} className="flex-shrink-0 p-2 rounded-full hover:bg-gray-100 transition-colors">
                <Heart size={22} className={saved ? "text-red-500 fill-red-500" : "text-gray-400"} />
              </button>
            </div>
            <p className="text-2xl sm:text-3xl font-bold text-brand-700 mt-1">{formatPrice(property.price, property.currency, property.price_period)}</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              ...
              { label: "Age", value: property.property_age_years ? `${property.property_age_years} yr old` : null },
          ...
          {/* Map placeholder */}
          <div className="rounded-xl overflow-hidden border border-gray-200 h-44 bg-gray-100 flex items-center justify-center text-gray-400 text-sm">
        ...
        {/* Seller card */}
        {property.seller && (
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm h-fit sticky top-16">
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 capitalize">{property.seller.role}</p>
              </div>
            </div>
            {property.seller.phone && (
              <>
                <a href={`tel:${property.seller.phone}`}
                  className="flex items-center justify-center gap-2 w-full bg-brand-600 text-white py-3 rounded-xl font-semibold mb-3 hover:bg-brand-700 transition-colors">
                  <Phone size={16} />
                  Call Now
                </a>
